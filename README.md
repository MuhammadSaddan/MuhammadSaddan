Muhammad Saddan
Email: muhammadsadaan789@gmail.com
LinkedIn: Muhammad Saddan

About Me
I am currently pursuing a master’s degree in Electrical Engineering at Yangzhou University. My research focuses on sensorless control of permanent magnet synchronous motors (PMSMs) using model-based methods for rotor position suppression in medium- to high-speed applications.

Research Interests
* Sensorless control of PMSMs
* Model-based rotor position and speed estimation
* Rotor position error suppression and compensation
* Electric motor drives
* Field-oriented control
* Control-system modeling and simulation
* Real-time implementation and experimental validation


Engineering Tools and Technologies
* MATLAB and Simulink

Education
Master’s Degree in Electrical Engineering — In Progress
Yangzhou University
Research area: Sensorless control, model-based estimation, and rotor position error suppression for PMSM drives.

BSc Electrical Engineering
University of Engineering and Technology, Lahore
2018–2022
